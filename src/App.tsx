  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/emails`,
        {
          method: "POST",
          headers: {
        ...
        <div className="w-full max-w-md p-8 rounded-lg backdrop-blur-sm mb-40">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label
                htmlFor="email"
                className="block text-sm font-medium text-white mb-1 mt-12 mr-96"
              >
                Email
              </label>
              <input
            ...
            {[
              {
                ...
                title: "Désordonné",
                description:
                  "Aucun cadre, aucune structure... Vous avez du mal à vous organiser ?",
              },
              {
                img: testt,
                title: "Débordé",
                description:
                  "Deadlines, tâches, notes, listes en tout genre… Vous n'arrivez plus à tout retenir ?",
              },
              {
                img: testtt,
                title: "Éparpillé",
                description:
                  "Posts-it, carnets, applications, logiciels... Vous auriez besoin de tout regrouper ?",
              },
            ].map((item, index) => (
              <div key={index} className="text-center">
                ...
                />
                <h4 className="text-3xl font-semi-bold text-white">
                  {item.title}
                </h4>
        ...
        <div className="container mx-auto max-w-7xl flex items-center">
          <div className="w-1/3 text-white pr-8">
            ...
            <p className="text-gray-300 mb-6 text-lg">
              Organisez tout, au même endroit. Plus besoin de jongler entre les
